...
def test_estimate_co_mole_fraction_returns_positive_values() -> None:
    temperature_k = np.array([2000.0, 2200.0])
    pressure_atm = np.array([1.0, 1.2])
    integrated_area_cm_inv = np.array([0.008, 0.010])

    co_mole_fraction = estimate_co_mole_fraction(
        temperature_k,
        pressure_atm,
        integrated_area_cm_inv,
        optical_path_length_cm=14.13,
    )

    assert np.all(co_mole_fraction > 0.0)
    assert np.all(co_mole_fraction < 1.0)
